/* form19260-g2.boot6307-g2.forms16725-g2663 */
/*!
...
(function($){
	...
	$.G2.forms.initializeEvents = function (thisForm){
		thisForm.find('[data-events]').each(function(i, inp){
			...
			jQuery.each(events, function(ei, event){
				//jQuery(inp).off('change click ready');
				jQuery(inp).on('change click ready', function($eve){
				...
				jQuery(inp).trigger('ready');
			});
		});
	}
	
	$.G2.forms.initializeFeatures = function (thisForm){
		thisForm.find('.partitioned .ui.button.green, .partitioned .ui.button.forward').off('click');
		thisForm.find('.partitioned .ui.button.green, .partitioned .ui.button.forward').on('click', function(e){
			e.preventDefault();
		...
		thisForm.find('.partitioned .ui.button.red, .partitioned .ui.button.back').off('click');
		thisForm.find('.partitioned .ui.button.red, .partitioned .ui.button.back').on('click', function(e){
			e.preventDefault();
			var activeTab = jQuery(this).closest('.partitioned').find('.ui.segment.tab.active').first();
			activeTab.find(':input').trigger('blur');
		...
		thisForm.find('.repeater .ui.button.green, .repeater .ui.button.multiply').off('click');
		thisForm.find('.repeater .ui.button.green, .repeater .ui.button.multiply').on('click', function(e){
			e.preventDefault();
			
			var cloned = jQuery(this).closest('.repeater').find('.ui.source-item').clone().show();
			cloned.find(':input').prop('disabled', false);
			
			cloned.html(cloned.html().replace(/-N-/g, jQuery(this).closest('.repeater').data('count')));
		...
		thisForm.find('.repeater .ui.button.red, .repeater .ui.button.remove').off('click');
		thisForm.find('.repeater .ui.button.red, .repeater .ui.button.remove').on('click', function(e){
			e.preventDefault();
	...
	$.G2.forms.ready = function(){
		jQuery('div[data-invisible="1"]').each(function(i, invForm){
			...
			var id = jQuery(invForm).attr('id');
			
			jQuery(invForm).html('');
			jQuery(invForm).after(jQuery('<p data-holderid="'+id+'"></p>'));
			var newFormHTML = jQuery('<p>').append(jQuery(invForm)).html().replace(/<div data-invisible="1"/, '<form data-invisible="1"').replace(/<\/div>/, '</form>');
			jQuery('p[data-holderid="'+id+'"]').replaceWith(jQuery(newFormHTML).html(content));
